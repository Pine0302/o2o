...
class StoresubLogic extends Model
{    
    ...
    {
        $user = D('Store_sub')->where(array('store_id' => $store_id))->relation($relation)->find();
        return $user;
    }
    ...
    public function addStore($store)
    {
        ...
        $store['user_id'] = 0;
        $store_id = Db::name('store_sub')->add($store);

		Db::name('store_sub_extend')->add(array('store_id'=>$store_id));
		...
		/*	$apply = array(
                ...
                // 'company_province'=>0,
                // 'company_province'=>$store['company_province'],
                // 'company_city'=>$store['company_city'],
                // 'company_district'=>$store['company_district'],
                'sc_bail'=>0,
                'apply_state'=>1,
			);
			M('store_sub_apply')->add($apply);*/
		}
		//添加店铺管理员
		$seller = array('seller_name'=>$store['seller_name'],'store_id'=>$store_id,'is_admin'=>1);
        $seller_id = Db::name('seller_sub')->add($seller);
		...
		if($store_id && $seller_id){
			...
			adminLog('新增分店：'.$store['seller_name']);
			return $store_id;
		}else{
			Db::rollback();
			return false;
