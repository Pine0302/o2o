...
class GoodsLogic extends Model
{
    ...
    public function saveGoodsPlus($goods_id,$plus_arr){
        if(count($plus_arr)>0){
            $arr = [];
            $create_time = $update_time = time();
            foreach($plus_arr as $kp=>$vp){
                $arr[] = [
                    'goods_id'=>$goods_id,
                    'plus_id'=>$vp,
            ...
            Db::name('goods_plus')->where('goods_id','=',$goods_id)->delete();
            Db::name('goods_plus')->insertAll($arr);
        }else{
            Db::name('goods_plus')->where('goods_id','=',$goods_id)->delete();
        }
    }
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $spec = M('Spec')->order('sort desc')->getField('id,name'); // 规格表
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
          ...
          // $str .="<td><input name='item[$item_key][commission]' value='{$keySpecGoodsPrice[$item_key][commission]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
            $str .="<td><input name='item[$item_key][store_count]' value='{$keySpecGoodsPrice[$item_key][store_count]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")'/><input type='hidden' name='item[$item_key][key_name]' value='$item_name' /></td>";
        ...
        if ($goods_item) {
            ...
            foreach ($goods_item as $k => $v) {
                ...
                M('cart')->where("goods_id = $goods_id and spec_key = '$k'")->save(array(
                    'market_price' => $market_price ?? 0, //$v['market_price'], //市场价
                    'goods_price' => $v['price'] , // 本店价
                    'member_goods_price' => $v['price'] ?? 0, // 会员折扣价
                ));
            }
            if($keyArr){
     ...
     * @param int $goods_id 商品id
     */
    public function afterSaveSpec($goods_id,$store_id)
    {
    //    $item_img = I('item_img/a');
        // 商品货号
     //   $goods_sn = "TP".str_pad($goods_id,7,"0",STR_PAD_LEFT);
   ...
   //     $eidt_goods_id = I('goods_id',0);
     ...
     * @return  mix
     */
    public function plus_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        global $attr_list;
        $sql = "SELECT * FROM  __PREFIX__plus_attr ORDER BY sort ASC";
        $attr_list = DB::query($sql);
        return $attr_list;
    }
}