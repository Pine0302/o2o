...
//        $this->assign('ad_position_list',$ad_position_list);//广告位
    ...
    public function adHandle(){
        ...
        if($data['act'] == 'add'){
            $r = D('adenter')->add($data);
        }
        if($data['act'] == 'edit'){
            $r = D('adenter')->where('ad_id', $data['ad_id'])->save($data);
        }

        if($data['act'] == 'del'){
            $r = D('adenter')->where('ad_id', $data['del_id'])->delete();
            if($r) exit(json_encode(1));
        }
        $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U('Admin/Adenter/adList');
        // 不管是添加还是修改广告 都清除一下缓存
        delFile(RUNTIME_PATH.'html'); // 先清除缓存, 否则不好预览

        if($r){
            $this->success("操作成功",U('Admin/Adenter/adList'));
        }else{
    ...
    public function ad(){
        ...
        $ad_info = array();
        if($ad_id){
            $ad_info = D('adenter')->where('ad_id',$ad_id)->find();
        ...
        $position = D('adenter_position')->where('1=1')->select();
    ...
    public function position(){
        ...
        $info = array();
        if($position_id){
            $info = D('adenter_position')->where('position_id='.$position_id)->find();
        }
    ...
    public function positionHandle(){
        ...
        if($data['act'] == 'del'){
        	if(M('adenter')->where('pid='.$data['position_id'])->count()>0){
    ...
    public function changeAdField(){
    	...
    	M('adenter')->save($data); // 根据条件保存修改的数据
    }
