...
//        $this->assign('ad_position_list',$ad_position_list);//广告位
    ...
    public function adHandle(){
        ...
        if($data['act'] == 'edit'){
            $r = D('ado')->where('ad_id', $data['ad_id'])->save($data);
        }

        if($data['act'] == 'del'){
            $r = D('ado')->where('ad_id', $data['del_id'])->delete();
            if($r) exit(json_encode(1));
        }
        $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U('Admin/Ado/adList');
        // 不管是添加还是修改广告 都清除一下缓存
        delFile(RUNTIME_PATH.'html'); // 先清除缓存, 否则不好预览

        if($r){
            $this->success("操作成功",U('Admin/Ado/adList'));
        }else{
    ...
    public function ad(){
        ...
        $ad_info = array();
        if($ad_id){
            $ad_info = D('ado')->where('ad_id',$ad_id)->find();
        ...
        $position = D('ado_position')->where('1=1')->select();
    ...
    public function position(){
        ...
        $info = array();
        if($position_id){
            $info = D('ado_position')->where('position_id='.$position_id)->find();
        }
    ...
    public function positionHandle(){
        ...
        if($data['act'] == 'del'){
        	if(M('ado')->where('pid='.$data['position_id'])->count()>0){
