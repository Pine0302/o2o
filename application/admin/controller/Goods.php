...
class Goods extends Base {
    ...
    public function addEditGoods()
    {
        ...
        if($goods_id){
          ...
          //  $brandList = $GoodsLogic->getSortBrands($goods['cat_id']);   //获取三级分类下的全部品牌
            $goodsSpecPlus = Db::name("goods_plus")->where('goods_id','=',$goods_id)->select();
        ...
        $goodsPlus = Db::name("PlusAttr")->select();
        foreach($goodsPlus as $kg=>$vg){
            $goodsPlus[$kg]['is_choose'] = 2;
        }
        if(count($goodsSpecPlus)>0){
            $plus_id_arr = [];
            foreach($goodsSpecPlus as $kp=>$vp){
                $plus_id_arr[] = $vp['plus_id'];
            }
            foreach($goodsPlus as $kg=>$vg){
                if(in_array($vg['id'],$plus_id_arr)){
                    $goodsPlus[$kg]['is_choose'] = 1;
                }
            }
        }
    ...
    //    $freight_template = Db::name('freight_template')->where('')->select();
     //   $this->assign('freight_template',$freight_template);
      //  $this->assign('suppliersList', $suppliersList);
        ...
        $this->assign('goodsPlus', $goodsPlus);
        $this->assign('goodsSpecPlus', $goodsSpecPlus);
        return $this->fetch('_goods');
    }


    //针对某个商品生成关联模型对应的默认规格
    public function handleDefaultSpecItem($data){
        $arr = [];
        $default_spec_item = '';
        foreach($data as $kd=>$vd){
            if(strpos($kd,'spec_id') !==false){
                $spec_id = substr($kd,8);
                $item_id = $vd;
                $arr[$spec_id] = $item_id;
                unset($data[$kd]);
        ...
        if(count($arr)>0){
            $default_spec_item = serialize($arr);
        }
        return $default_spec_item;
    }

    //获取商品加料列表
    public function goodsPlusTemplate(){
    ...
    public function save(){
        ...
        //查找所有的分公司
        $sub_store_list = Db::table('tp_store_sub')->field('store_id')->select();
      ...
      //  $GoodsLogic->saveGoodsPlus($goods['goods_id'], $plus_item); // 处理商品 加料
        ...
        if(!$data['goods_id']){
            //获取所有分店
            $store_list = Db::name('store_sub')->field('store_id,store_name')->select();
            if(count($store_list)>0){
                foreach($store_list as $ks=>$vs){

                    $data['parent_id'] = $ori_goods_id;
                    $data['store_id'] = $vs['store_id'];
                    $goods = new \app\common\model\Goods();
                    $goods->data($data, true);
                    $result = $goods->save();
                    $GoodsLogic->afterSaveSpec($goods['goods_id'],$goods['store_id']);
                    $GoodsLogic->saveGoodsPlus($goods['goods_id'], $plus_item); // 处理商品 加料
            ...
            $sub_goods_list = Db::name('goods')->where('parent_id','=',$goods_id)->field('goods_id,store_id')->select();

            foreach($sub_goods_list as $ks=>$vs){
                $data['goods_id'] = $vs['goods_id'];
                $goods = \app\common\model\Goods::get($vs['goods_id']);
         //       var_dump($data);exit;
                unset($data['goods_sn']);
              /*  var_dump($goods['goods_id']);
                ...
                $result = $goods->save();
             //   var_dump($goods);
                $GoodsLogic->afterSaveSpec($vs['goods_id'],$vs['store_id']);
                $GoodsLogic->saveGoodsPlus($vs['goods_id'], $plus_item); // 处理商品 加料
            }
        }
        $this->ajaxReturn($return_arr);
    }
    ...
    //产品加料管理
    public function plusGoods($plus_ids,$good_id){
        //先删除对应商品的加料规格
        $now = time();
        Db::table('tp_goods_plus')->where('goods_id','=',$good_id)->delete();

        //重新部署加料规格
        if(count($plus_ids)>0){
            foreach($plus_ids as $kp=>$vp){
                $data[] = [
                    'goods_id'=>$good_id,
                    'plus_id'=>$vp,
                    'create_time'=>$now,
            ...
            Db::table('tp_goods_plus')->insertAll($data);
        }
    }
    ...
    public function saveType()
    {
        ...
        foreach($data['spec'] as $ks=>$vs){
            $data['spec'][$ks]['sort'] = $vs['order'];
        }
        ...
        try{
            ...
            Db::name('spec')->query("update tp_spec set sort = `order`");
    ...
    public function delGoods()
    {
        ...
        $childGoodsList = M("Goods")->whereIn('parent_id',$goods_ids)->select();  //商品表
        $childGoodsIds = '';
        foreach($childGoodsList as $kc=>$vc){
            $childGoodsIds = $childGoodsIds.$vc['goods_id'].",";
        }
        $childGoodsIds = rtrim($childGoodsIds,",");
        $goods_ids = $goods_ids.",".$childGoodsIds;
     ...
     *  商品加料列表
     */
    public function plusList(){
        $GoodsLogic = new GoodsLogic();
        $attr_list = $GoodsLogic->plus_list();
    ...
    public  function addEditPlus(){
        ...
        if(IS_POST)
        {
            ...
            $attrVilidate = Loader::validate('GoodsPlus');
            if(!$attrVilidate->batch()->check($data)){
                $return = ['status'=>0,'msg'=>'操作失败','result'=>$attrVilidate->getError()];
                $this->ajaxReturn($return);
            }
            if($id){
                Db::name("PlusAttr")->update($data);
            }else{
                Db::name("PlusAttr")->insert($data);
            }
            $this->ajaxReturn(['status'=>1,'msg'=>'操作成功','result'=>'']);
        }
        $attr = M("plusAttr")->find($id);
        $this->assign('attr',$attr);
        return $this->fetch('_plus');
    }
     ...
     * 删除配料
     */
    public function delPlus()
    {
        $ids = I('post.ids','');
        empty($ids) && $this->ajaxReturn(['status' => -1,'msg' => '非法操作！']);
        $ids = rtrim($ids,",");
        $res=Db::name('PlusAttr')->whereIn('id',$ids)->delete();
        if($res){
            $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/Goods/plusList")]);
        }
        $this->ajaxReturn(['status' => -1,'msg' => '操作失败','data'  =>'']);
    }
