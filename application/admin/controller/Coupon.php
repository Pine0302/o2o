...
class Coupon extends Base {
     ...
     * 添加编辑一个优惠券类型
     */
    public function coupon_info_bak(){
        $cid = I('get.id/d');
        if ($cid) {
    ...
    */
    ...
    public function del_coupon(){
        ...
        $cid = I('get.id/d');
        //查询是否存在优惠券
        $row = M('plat_coupon')->where(array('id'=>$cid))->delete();
        if (!$row) {
     ...
     //   M('coupon_list')->where(array('cid'=>$cid))->delete();
        M('plat_coupon')->where(array('id'=>$cid))->delete();
    ...
    public function index(){
        ...
        //is_brand_store：0：区县代理门店优惠劵；1：品牌商代金券；
        $map['is_brand_store'] = 1;

        $count =  M('plat_coupon')->where($map)->count();
        $Page = new Page($count,10);
        $show = $Page->show();
        $lists = M('plat_coupon')->where($map)->order('add_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        foreach($lists as $kl=>$vl){
            $lists[$kl]['is_new_coupon'] = ($vl['is_new']==1) ? "是":"否";
            $lists[$kl]['is_rec_coupon'] = ($vl['is_rec']==1) ? "是":"否";
            if($vl['type']==2){
                $lists[$kl]['money'] = ($vl['money']/10)."折";
            }
        }
...
//        $this->assign('store',$store);
    ...
    public function newin_coupon(){
        ...
        //把原来新人券取消
        if(!empty($cid)){
            /*$ori_new_list = Db::table('tp_plat_coupon')->where('is_new','=',1)->select();
            if(count($ori_new_list)>0){
                foreach($ori_new_list as $ko=>$vo){
                    $row = M('plat_coupon')->where(array('is_new' => 1))->save(['is_new'=>2]);
                }
            }*/
        }
        //将该代金券设置为新人券
        $result = M('plat_coupon')->where(array('id' => $cid))->save(['is_new'=>1]);
        if($result){
            //删除此类型下的代金券
            //M('coupon_list')->where(array('cid'=>$cid))->delete();
            $msn = array('status'=>1,'msg'=>'设置成功');
        }else{
            $msn = array('status'=>0,'msg'=>'设置失败');
        }
        $this->ajaxReturn($msn);
    }
   ...
   * 取消设置代金券为新人券
   */
    public function un_newin_coupon(){
        //获取代金券ID
        $cid = I('get.id');
        //$row = M('plat_coupon')->where(array('id'=>$cid))->delete();
        ...
        //将该代金券设置为非新人券
        $result = M('plat_coupon')->where(array('id' => $cid))->save(['is_new'=>2]);
        if($result){
            //删除此类型下的代金券
            //M('coupon_list')->where(array('cid'=>$cid))->delete();
            $msn = array('status'=>1,'msg'=>'设置成功');
 ...
 */
    public function rec_coupon(){
        ...
        //把原来新人券取消
        if(!empty($cid)){
            /*$ori_new_list = Db::table('tp_plat_coupon')->where('is_rec','=',1)->select();
            if(count($ori_new_list)>0){
                foreach($ori_new_list as $ko=>$vo){
                    $row = M('plat_coupon')->where(array('is_rec' => 1))->save(['is_rec'=>2]);
                }
            }*/
        }
        //将该代金券设置为新人券
        $result = M('plat_coupon')->where(array('id' => $cid))->save(['is_rec'=>1]);
        if($result){
            //删除此类型下的代金券
            //M('coupon_list')->where(array('cid'=>$cid))->delete();
            $msn = array('status'=>1,'msg'=>'设置成功');
...
*/
    public function un_rec_coupon(){
        ...
        //将该代金券设置为新人券
        $result = M('plat_coupon')->where(array('id' => $cid))->save(['is_rec'=>2]);
        if($result){
            //删除此类型下的代金券
            //M('coupon_list')->where(array('cid'=>$cid))->delete();
            $msn = array('status'=>1,'msg'=>'设置成功');
    ...
    public function send_coupons(){
        $cid = I('id');
        $now = time();
        $orderLogic = new CouponLogic();

        $user_list = Db::table('tp_users')
            ->where('mobile','not null')
          //  ->where('user_id','=','265')
            ->whereOr('weixin_mobile','not null')
           // ->field('user_id')
            ->select();

        $coupon_info = Db::table('tp_plat_coupon')->where('id','=',$cid)->find();
        if(count($user_list)>0){
            foreach($user_list as $ku=>$vu){
                    $arr[] = [
                        ...
                        'method'=>config('COUPON_METHOD')['ADMIN'],
                        'code'=>$orderLogic->get_coupon_sn($vu['user_id']),
                        'send_time'=>$now,
                        'status'=>0,
                    ];
            }
            $result = Db::table('tp_coupon_list')->insertAll($arr);

            if($result){
                Db::name('plat_coupon')->where('id',$cid)->setInc('send_num',$result);
                $msn = array('status'=>1,'msg'=>'发送成功');
            }else{
                $msn = array('status'=>0,'msg'=>'发送失败');
            }
        }else{
            $msn = array('status'=>1,'msg'=>'暂无可发送会员');
        }
        $this->ajaxReturn($msn);
    }
    ...
    public function coupon_info(){
        if (IS_POST) {
            ...
            if(!empty($data['category'])){
                $arr_cate = array_values($data['category']);
                $category_str = implode(",",$data['category']);
            }



            $data['category_ids'] = $category_str;
            $data['send_start_time'] = strtotime($data['send_start_time']);
            $data['send_end_time'] = strtotime($data['send_end_time']);
            $data['use_end_time'] = strtotime($data['use_end_time']);
            $data['use_start_time'] = strtotime($data['use_start_time']);
            ...
            if (!$data['id']) {
                $data['add_time'] = time();
                $data['is_brand_store'] = 1;
                if($data['type']==2){
                    $data['condition']=0;
                }
                $row = M('plat_coupon')->add($data);
                if (!$row) {
                    $this->error('添加优惠券失败');
                } else {
                    $this->success('添加优惠券成功', U('Coupon/index'));
                }
                exit;
            } else {
                $row = M('plat_coupon')->where(array('id' => $data['id']))->save($data);
                if (!$row) {
                    $this->error('编辑优惠券失败');
                } else {
                    $this->success('编辑优惠券成功', U('Coupon/index'));
                }
                exit;
            }
        ...
        $cid = I('get.id/d');
        $goodCategory = Db::name("GoodsCategory")->select();
        foreach($goodCategory as $kg=>$vg){
            $goodCategory[$kg]['is_choose'] = 2;
        }
        $category_arr = [];
        if ($cid) {
            //is_brand_store：0：区县代理门店优惠劵；1：品牌商代金券；
            $coupon = M('plat_coupon')->where(array('id' => $cid))->find();
        /*    $coupon['use_start_time'] = date("Y-m-d",$coupon['use_start_time']);
           ...
           foreach($goodCategory as $kg=>$vg){
                if(in_array($vg['id'],$category_arr)){
                    $goodCategory[$kg]['is_choose'] = 1;
                }
           }
        ...
        $this->assign('category', $goodCategory);
