...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        $end = $this->end;
        // 搜索条件
        //$condition = array('order_status'=>0);
   ...
   /*     if(I('pay_code')){
    ...
    public function receive_order(){
        ...
        Db::name('order')->where('order_id','=',$order_id)->update($update_arr);
        $order_info =  Db::name('order')->where('order_id','=',$order_id)->find();
        //$data = $this->getOrderInfoStatus($order_id);
        $info = $this->ajaxReturn(['status' => 1, 'msg' => '接单成功','order_info'=>$order_info]);
     //   $this->ajaxReturn(['status' => 1, 'msg' => '接单成功','data'=>$info,'op'=>$op]);
    }

    public function getOrderInfoStatus($order_id){
        $order_info = Db::name('order')->where('order_id','=',$order_id)->find();
        $content = '';
        $op = '';
        if($order_info['type']==1){  //自取
    ...
    public function send_order(){
        ...
        //$order
        //生成顺丰订单
        $this->createSfOrder($order_id);

      /*  $noticeObj = new NoticeHandle();
        $noticeObj->sendModel($order_id,4);*/
        Db::name('order')->where('order_id','=',$order_id)->update($update_arr);
    ...
    public  function can_get_order(){
        ...
        $noticeObj->sendModel($order_id,3);
        //生成顺丰订单
     //   $this->createSfOrder($order_id);
        Db::name('order')->where('order_id','=',$order_id)->update($update_arr);
        $this->ajaxReturn(['status' => 1, 'msg' => '接单成功']);
    ...
    //可取单
    public  function finish_self_order(){
        $order_id = I('post.order_id/d',0);
        $update_arr = [
            'order_status'=>5,
            'cook_time'=>time(),
        ...
        $noticeObj = new NoticeHandle();
        //print_r($order_id);exit;
        $noticeObj->sendModel($order_id,5);
        //生成顺丰订单
        //   $this->createSfOrder($order_id);
        Db::name('order')->where('order_id','=',$order_id)->update($update_arr);
    ...
    //客户取单之后完成
/*    public  function can_get_finish_order(){
        $order_id = I('post.order_id/d',0);
        $update_arr = [
            'order_status'=>3,
            'cook_time'=>time(),
    ...
    //生成顺丰订单
    public function createSfOrder($order_id){
        ...
        $result_pre_create = $openicSfObj->preCreateSfOrder($order_info,$order_goods,$store_info);
    //    if($order_id==311){
    //        print_r($result_pre_create);exit;
       // }
       // print_r($result_pre_create);exit;
        $expect_time = 0;
        if(!empty($result_pre_create['expect_time'])){
            $expect_time = $result_pre_create['expect_time'];
            Db::name('order')->where('order_id','=',$order_id)->update(['sf_expect_time'=>$expect_time]);
        }
    ...
    public function cancelRiderH5($order_id){
        $order_info = Db::name('order')->where('order_id','=',$order_id)->find();
        $openicSfObj = new openicSf();
        $result = $openicSfObj->cancelRider($order_info);
        if(!empty($result['shop_order_id'])){
            $arr_order_update = [
                'order_status'=>7,
                'cancel_time'=>time(),
            ];
            Db::name('order')->where('order_sn','=',$result['shop_order_id'])->update($arr_order_update);
        }
    ...
    public function export_order()
    {
        ...
        if(is_array($orderList)){
            ...
            foreach($orderList as $k=>$val){
                ...
                $strTable .= '<td style="text-align:left;font-size:12px;">'."{$val['address']}{$val['address_num']}".' </td>';
                ...
                foreach($orderGoods as $goods){
                    ...
                    if ($goods['key_name'] != '') $strGoods .= " 规格：".$goods['key_name'];
                    $strGoods .= "<br />";
                }
                unset($orderGoods);
