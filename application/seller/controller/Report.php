...
class Report extends Base
{
    ...
    public function index(){
        ...
        $today['today_amount'] = M('order')->where("add_time>$now AND (pay_status=1 or pay_code='cod') and order_status in(1,2,3,4,5) and store_id = $this->store_id ")->sum('total_amount');//今日销售总额
        $today['today_order'] = M('order')->where("add_time>$now and (pay_status=1 or pay_code='cod') and order_status not in (0,6,7) and store_id = $this->store_id ")->count();//今日订单数
        $today['cancel_order'] = M('order')->where("add_time>$now AND order_status=7 and store_id = $this->store_id ")->count();//今日取消订单
